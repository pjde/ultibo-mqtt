...
//    mtReserved0,	  //  0	Reserved
  ...
  public
    ...
    procedure Parse (Buff : pointer; count : integer); overload;
...
procedure TMQTTParser.Parse (Buff : pointer; count : integer);
var
  aStream : TMemoryStream;
begin
  aStream := TMemoryStream.Create;
  aStream.Write (Buff^, count);
...
//                        log ('publish "' + aStr + '"   "' + display (s) + '"');
                        if Assigned (FOnPublish) then FOnPublish (Self, id, aStr, s);
                      end;
                  mtPUBACK,
                  mtPUBREC,
                  mtPUBREL,
...
begin
  ...
  AddByte (s, anID div $100);
  AddByte (s, anID mod $100);
  for i := 0 to Topics.Count - 1 do AddStr (s, UTF8String (Topics[i]));
  // payload
  s.Seek (0, soFromBeginning);
  AddLength (FTxStream, s.Size);
  FTxStream.CopyFrom (s, s.Size);
